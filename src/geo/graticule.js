...
d3.geo.graticule = function() {
  ...
  graticule.points = function() {
    var PX = [X0].concat(d3.range(Math.ceil(X0 / DX + ε) * DX, X1 - ε, DX), X1),
        PY = [Y0].concat(d3.range(Math.ceil(Y0 / DY + ε) * DY, Y1 - ε, DY), Y1),
        px = [x0].concat(d3.range(Math.ceil(x0 / dx + ε) * dx, x1 - ε, dx), x1).filter(function(x) { return Math.abs(x % DX) > ε; }),
        py = [y0].concat(d3.range(Math.ceil(y0 / dy + ε) * dy, y1 - ε, dy), y1).filter(function(y) { return Math.abs(y % DY) > ε; });
    return {
      type: "MultiPoint",
      coordinates: d3_geo_graticuleCross(PX, PY)
        .concat(d3_geo_graticuleCross(PX, py),
          d3_geo_graticuleCross(px, PY),
          d3_geo_graticuleCross(px, py))
    };
  };
